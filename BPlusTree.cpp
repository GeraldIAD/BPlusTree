...
void BPlusTree<Key, Value>::insert(Key k, Value val)
{
    ...
    { // 找到要插入的叶子节点cur
        int i = 0;
        while (i < cur->indices.size() && k > cur->indices[i])
            i++;
        cur = cur->children[i];
    }
    if (cur->values.size() < MAX_IND)
    { // 叶子节点未满，直接插入
        int i = 0;
        while (i < cur->indices.size() && k > cur->indices[i])
            i++; // 找到插入位置，插入键值对
    ...
    { // 当前叶子节点已满，分裂为cur和new_node两个节点
        ...
        { // 原节点不是根节点，递归调整父节点
            ...
            int i = 0;
            while (i < parent->children.size() && parent->children[i] != cur)
                i++;
            // 把新节点的第一个键插入到父节点中
            ...
            if (parent->indices.size() > MAX_IND)
            { // 父节点超出，递归分裂
                printf("父节点超出，递归分裂\n");
                split(parent);
            }
...
void BPlusTree<Key, Value>::remove(Key k)
{
    ...
    if (cur->indices.size() < MIN_IND)
    {
        ...
        else if (right != NULL && right->indices.size() > MIN_IND)
        {
            ...
            // 把父节点的索引值更新为借来的键值对
            int i = 0;
            while (i < parent->indices.size() && parent->indices[i] != borrow_key)
                i++;
        ...
        else
        {
            ...
            else if (right != NULL && right->parent == cur->parent)
            {
                ...
                int i = 0;
                while (i < parent->children.size() && parent->children[i] != right)
                    i++;
                parent->children.erase(parent->children.begin() + i);
                parent->indices.erase(parent->indices.begin() + i - 1);
                // 调整指针
...
void BPlusTree<Key, Value>::adjust(Node<Key, Value> *node)
{
    ...
    if (left != NULL && left->indices.size() > MIN_IND)
    {
        ...
        // 把父节点的索引值更新为借来的键值对
        int j = 0;
        while (j < parent->indices.size() && parent->indices[j] != borrow_key)
            j++;
